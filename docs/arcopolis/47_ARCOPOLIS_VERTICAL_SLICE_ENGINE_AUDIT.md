...
- **Final, consolidated audit.** This is the single deliverable. It supersedes the earlier draft passes; the
  ...
  *Provenance* (end). One earlier draft claim — that the bump-melee path does not arm the technique-prompt
...
success signal; the *shape* of "return success" is formalized as **Spike 2** (§9) — and is **not** a thing
...
| Ramps (auto-z inside planar `move`)      | `TFLAG_RAMP_UP` → `dest_loc.z()+=1; via_ramp=true`; `TFLAG_RAMP_DOWN`/ladder → `dest_loc.z()-=1; via_ramp=true` (`src/avatar_action.cpp:390-397`); ramp ids `terrain-zlevel-transitions.json:246-276` | NATIVE-BN (reachable via `move`; ramp branch prompt-free; **end-to-end unwitnessed**) | Yes (ramp tower) | **Resolved this session:** the ramp branch (`:390-397`) adds **no prompt**, sets the destination to the same x/y at `z±1`, and hands off to `g->walk_move(dest_loc, via_ramp)` (`:682`,`:730`). So ramp traversal reduces to a normal planar `walk_move` carrying a `via_ramp` flag onto a z-changed tile (`RAMP_END` is the landing-side flag, `:216`). The only residual is whether `walk_move` itself prompts on the z-changed dest — the same question as any planar `move`, which Arcopolis already drives. A one-command probe confirms; no longer "fully unverified." |
...
for Stage A: **Spike 1's fixture generator must assert** (1) a correctly-flagged stair tile aligned
**directly above/below** the avatar (hits the fast path and pins a deterministic destination tile), **and**
(2) **no creature on the destination stairs tile.** Both hold ⇒ no fabrication, no `query_yn`, deterministic.

**What could silently fail / overclaim:** single-z snapshot makes a multi-floor complex *look* one floor
...
**Easiest first proof.** Save-edit a `briefcase` onto a windowed tile, assert `entities.items[]` shows it,
`pickup` removes it. Reuses witnessed item-export + pickup paths with **no new engine code.**
...
- **Ranged attack (NATIVE-BN, citation closed this session).** `mon_secubot` carries a full `gun`
  ...
  no Arcopolis command for the *enemy*.
...
- **Placement (NATIVE-BN).** Mapgen `place_monster`/`place_monsters` (`src/mapgen.cpp:3861,3866`); example
  `data/json/mapgen/Bastion_Fort.json:215`. Deterministic single monster via `make_monster_fixture.py`
  (`TEST_FIXTURES.md:46-52`).
- **Patrol / building alarm (NOT FOUND — searched this session, scoped negative).** No *scripted* monster
...
- **Technique-prompt suppression — corrected this session (an earlier draft had this backwards).** The
  ...
  (`:974-976`). And `manual_combat_mode` **defaults to `false`** (`src/game.h:1196`), is set false on
  construction (`src/game.cpp:940,3984`), and is **forced false on save load** (`src/savegame.cpp:366`). The
  guard is queried at exactly one site, `src/melee.cpp:1593`; when armed, `is_technique_prompt_suppressed()`
...
**Best first security entity (resolved this session).** For a *clean, deterministic bump-melee witness* the
recommendation is **`mon_turret_searchlight`** (`data/json/monsters/turrets.json:3`): `IMMOBILE` (`:29`,
predictable position), `hp 30` (`:12`, a few bumps), and its only special is `SEARCHLIGHT` (`:26`) — **no gun
or melee retaliation**, so it cannot kill the avatar mid-fixture. Alternative melee target: **`mon_manhack`**
(`drones.json:101`, melee-only, `melee_cut 12`) — but `speed 190` + `HIT_AND_RUN` (`:120`) means it kites,
which is less deterministic. **Avoid for the first witness:** `mon_secubot` (gun, range 30, `speed 40`,
lethal), `mon_laserturret` (gun + `RETURN_FIRE`, `turrets.json:54-75`), `mon_riotbot` (`ZAPBACK` damages the
melee attacker + relaxation gas), `mon_copbot` (`hp 80` = many bumps + `COPBOT` handcuff special).
...
| **Unguarded route (avoid the fight)**    | Planar movement witnessed; enemies in `entities.monsters[]`                                                            | NATIVE-BN (planar move witnessed; the *route composition* is not) | **Stage A**       |
...
or **unguarded** paths around a static threat — both *reachable through* the witnessed `move`, though
neither route composition has its own Arcopolis fixture yet. **Silent-failure caveat:** headless
`seen=false` hides player LOS, so stealth *outcomes* are engine-computed but not snapshot-observable; Stage
...
- Fixture: two floors, **aligned** ordinary stairs (`t_stairs_up`/`t_stairs_down`) placed **directly
  above/below** the avatar; no rope, climb prompt, elevator, vehicle, NPC, neutral creature, or monster on
  the stairs tile. **The generator must assert both conditions from §4 before the run** — a misaligned or
  creature-occupied stair would trigger `find_or_make_stairs` fabrication (`src/game.cpp:14911-14914`) or the
  push-past `query_yn` (`:14885-14887`), perturbing determinism and possibly failing loud.
- Assert: command succeeds on aligned stairs; avatar `z` changes; **the post-move snapshot reports the new
  `z` and the new floor's tiles/entities** (this is the thing to *prove*, not assume — §4); a unit-level
...
a new 2-floor save fixture + generator, and a new `vertical_move_regression.ps1`.
...
| **Frontend-side heuristic**         | Package left ground + avatar at contact tile ⇒ *consider* success.      | Fastest, weakest; cannot prove the package is still carried (could be dropped/given away).                    |
...
> Arcopolis can prove "the package is currently carried by the avatar" for the witnessed fixture using
> <chosen design>; a frontend determines "package returned" from <chosen design> + the avatar's exported
> position. (If the heuristic design is chosen, the doc must label this signal as weaker than engine truth.)

### Lower-risk warm-up witnesses

- **Package fixture on ground (smallest; do this even before Spike 1).** `briefcase` save-edited onto a
  windowed tile; witness `entities.items[]` + `pickup` removal. No new engine code.
- **Hostile-drone observation fixture.** `mon_turret_searchlight` (clean, immobile, non-lethal) or
  `mon_secubot` (ranged message) save-edited on passable terrain; `wait`; witness presence. No equivalence
  claim.
...
- **No "generic vertical movement"** — Spike 1 proves *aligned-stair* up/down only.
...
- **No "package returned"** from snapshot position alone unless explicitly labeled a *frontend heuristic*.
...
- **No "bump-melee is *unconditionally* technique-prompt-free."** It is suppressed only while
  `manual_combat_mode` is off — which is the default and the save-load value (§6). The fixture must pin
  `manual_combat_mode == false` and still fail loud on any prompt; do not assume a player who toggled manual
  combat mode would see the same behavior.
- **No "drivable now" without "but unwitnessed"** for ramps, door-auto-open, bump-melee, and route
  compositions — all are *reachable through* the witnessed `move` command but have no fixture of their own.
...
3. **Security pressure (observation first):** save-edit a hostile drone on the direct route; witness it in
   `entities.monsters[]`; route around it (uses witnessed `move`).
4. **Optional fight witness:** a separate fixture where `move` into the hostile drone resolves as melee
   (`mon_turret_searchlight` is the cleanest target, §6); pin `manual_combat_mode == false`; assert `hp`
   change **and no unexpected prompt** (the bump path is prompt-suppressed in default play, §6).
...
- **Best first security entity** — `mon_turret_searchlight` (immobile, non-lethal); avoid secubot/laserturret/
...
| `game::vertical_move` is prompt-heavy                                                                       | `src/game.cpp:14076,14128`                                                                             | NATIVE-BN                                | Which prompts fire in clean stair case (Q1).                                                     |
...
| **Bump-melee IS technique-prompt-suppressed in default play** (delegate arms the guard while `manual_combat_mode` is off) | guard via delegate `src/avatar_action.cpp:181-184,971-981`; default `src/game.h:1196`, load `savegame.cpp:366`; single check site `src/melee.cpp:1593` | NATIVE-BN (corrects earlier draft)       | Fixture pins `manual_combat_mode=false` + fail-loud (resolved).                                  |
| `mon_turret_searchlight` is the cleanest bump witness (immobile, hp 30, non-lethal)                          | `data/json/monsters/turrets.json:3,12,26,29`                                                           | NATIVE-BN                                | Avoid secubot/laserturret/riotbot/copbot first (resolved).                                        |
...
| --- | --- | --- | --- |
| Bump-melee technique prompt | "guard NOT armed on bump path → prompt likely fires; witness may need `autoattack`" — **wrong** | Bump delegates to `melee_attack_while_handling_manual_combat_mode`, which **arms** the guard while `manual_combat_mode` is off (default + forced false on load). Suppressed in default play. | `avatar_action.cpp:181-184,971-981`; `game.h:1196`; `game.cpp:940,3984`; `savegame.cpp:366`; `melee.cpp:1591-1607` |
| `find_or_make_stairs` determinism | hazard flagged; safe condition not pinned | Fabricates only when `find_stairs` returns `nullopt`; aligned-above/below stair + no creature on the tile avoids fabrication *and* the push-past prompt | `game.cpp:14835,14840-14846,14885-14887,14896,14904-14914` |
...
| Best first security entity | open (manhack vs secubot vs turret) | `mon_turret_searchlight` (immobile, hp 30, non-lethal SEARCHLIGHT); avoid ranged/retaliating/high-hp bots first | `turrets.json:3,12,26,29,54-75`; `drones.json:101,107-120`; `defense_bot.json:89-135` |
