...
BN's engine and content support a 5–6 floor complex; Arcopolis can now **drive explicit vertical
movement** (Spike 24 — the `vertical_move` verb, matched-stair only, §9) but cannot yet **observe more than
one floor at a time**. The `hotel_1` floor count is stated precisely below,
...
| Stairs vertical primitive           | `game::vertical_move` (`src/game.cpp:14076`); `find_stairs:14835`; `find_or_make_stairs:14896`                                                                                                        | NATIVE-BN                                                                             | Yes              | **Drivable since Spike 24** via the `vertical_move` verb (`down`/`up` → `ACTION_MOVE_DOWN`/`UP` → native `game::vertical_move`), matched-stair fast path only, level 2/3 — doc 49 (§9).                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Arcopolis vertical command          | `vertical_move` verb → `ACTION_MOVE_DOWN`/`UP` → native `game::vertical_move`; IMPLEMENTED Spike 24 (`src/arcopolis_command.cpp:71-83,272-295`)                                                       | NEEDS NEW SEAM                                                                        | n/a              | **Yes** — drivable matched-stair, level 2/3 (Spike 24, doc 49).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| --------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Vertical move up/down (stairs)    | `game::vertical_move`; `ACTION_MOVE_UP/DOWN`; token table | `src/game.cpp:14076`; `src/handle_action.cpp:2184-2307`; `src/action.cpp:657-660`                                                                                                                       | NATIVE-BN (Spike 24)                                                                                  | **Done** — the `vertical_move` verb drives it (matched-stair, level 2/3; doc 49). Mind `find_or_make_stairs` determinism (§4). |
...
> spike) was **split across two global spikes**: **Spike 23** created the 2-floor aligned-stair
> fixture (`ArcopolisStairsTest`, built by `make_stairs_fixture.py`), and **Spike 24 (DONE)** added the
> vertical-movement witness that drives it — implemented as the **`vertical_move`** verb
> (`direction: "down" | "up"`), **NOT** the `move_up` / `move_down` verb names sketched in the Scope below
> (the API decision is recorded in [49_SPIKE24_VERTICAL_MOVEMENT_WITNESS.md](49_SPIKE24_VERTICAL_MOVEMENT_WITNESS.md)).
> "Spike 2" below maps to whatever later global spike ships the carried/return signal. See
...
**Status: DONE.** Spike 23 created the fixture only (`ArcopolisStairsTest`); **Spike 24 proved
Arcopolis-driven vertical movement through it** — the `vertical_move` verb (`down`/`up` →
`ACTION_MOVE_DOWN`/`ACTION_MOVE_UP` → native `game::vertical_move`), a matched-stair down → up round trip at
level 2/3, gated by `vertical_movement_regression.ps1`. Record:
[49_SPIKE24_VERTICAL_MOVEMENT_WITNESS.md](49_SPIKE24_VERTICAL_MOVEMENT_WITNESS.md). The Scope/Acceptance
below is the original forecast; doc 49 is the authoritative as-built record (and the chosen API is
`vertical_move`, not the `move_up`/`move_down` verbs sketched here).

**Equivalence level (conservative).** (The numbered **equivalence levels 1–4** are defined in
`AGENTS.md:111-120`, restated at [ARCOPOLIS_STATE.md](ARCOPOLIS_STATE.md) §Terminology →
"Equivalence levels (1–4)".) `ACTION_MOVE_UP/DOWN` reach the engine through `handle_action`'s real
...
| Arcopolis drives vertical move (`vertical_move`, Spike 24); snapshot single-z                                                                                        | `src/arcopolis_command.cpp:60-77`; `src/arcopolis_export.cpp:180-185`                                                                                                        | NEEDS NEW SEAM                                                           | Vertical move PROVEN (Spike 24, matched-stair, level 2/3, doc 49); per-floor observation now witnessed; simultaneous multi-z not built.                    |
