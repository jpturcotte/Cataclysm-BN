# Arcopolis backend — current state (truth as of the Spike 29A fight-mechanic witness, 2026-07-04)
...
[49_SPIKE24_VERTICAL_MOVEMENT_WITNESS.md](49_SPIKE24_VERTICAL_MOVEMENT_WITNESS.md). **The folded Spike 28
...
## Spike-number registry (maintainer convention, 2026-07-05)

Spike numbers are assigned in SHIP order; letter suffixes mark sub-spikes of one effort (the 27A/27B
shape); **no numbers are skipped except failures** (Spike 3). A **fold takes its earliest constituent
number**: the merged 2-floor + N-floor composite is **Spike 28** (it shipped titled "Spike 29" — PR #95's
git title is immutable; doc 61 carries the errata). The fight-mechanic witness is **Spike 29A** (doc 62).
Doc 60's proposal-time labels map: its 28 and 29 → **28**; its 31 (fight) → **29A**; its 30 (drone) and
32 (sneak) take real numbers when carded — the drone half is the natural **29B** sibling of 29A. Doc 60
itself stays deliberately unedited; read its numbers as proposal labels through this mapping.
...
| 28    | **Folded vertical-slice composite** (doc 61; the separate Spike-28 proposal folded in 2026-07-01; the merged spike RENUMBERED 29→28 on 2026-07-05 per the registry note above — it SHIPPED titled "Spike 29" in PR #95, whose git title is immutable) — FOUR claims, no composite headline: **C1** per-floor-pair fixture determinism (the parameterized `make_stairs_fixture.py --floors N` asserts doc 47 §4's three conditions per pair via the stair terrain IDS + the `.sav` monster scan — the honest bound is doc 61 §5; lower floors SYNTHESIZED by footprint-wide row INSERT — probe-proven; default invocation stays content-identical, gated); **C2** a 6-floor z=0…−5 descent→ascent round trip (18 legs, per-leg pos/ter/turn; L1 observation of the level-2/3 `vertical_move`); **C3** the 2-floor composite — descend → walk → level-4 `pickup` → return → ascend → consumer-side `carried_at_contact` conjunction green, with the doc-53 guards + floor provenance + the PINNED z-changed off-contact guard (contact x/y, z −1 — a z-blind conjunction fails it); **C4** the same composite at 6 floors, package on z=−5 (Stage A's traversal core). Arcopolis-layer **L1 composite** (docs 53/55), per-verb levels unchanged (pickup B/L4 at its witnessed site, `has_item` C, position S, vertical 2/3); **NO new equivalence claim, zero `src/`**. G3/G4 are also the FIRST live-transport `vertical_move` witnesses. Fixtures `ArcopolisSliceTest`/`ArcopolisTowerTest` (clones of `ArcopolisBackpackTest` — probe-decided: `box_small` doesn't fit the stock avatar), driver `slice_live_driver.py`, gate `slice_regression.ps1` (G1–G4) | ✅                                      |
| 29A   | **Fight-MECHANIC witness (avatar bump-melee)** — a driven `move` into an ADJACENT hostile `mon_zombie` routes through the engine's own bump-melee (`avatar_action::move` → `melee_attack_from_movement` → `Character::melee_attack`; the technique prompt is suppressed IN-ENGINE on the `manual_combat_mode==false` path, `avatar_action.cpp:984`) and drops the zombie's exported `entities.monsters[].hp`. **L1 observation, class A (stimulus — the move verb stays level-2/3 injected) via S (raw `monster::get_hp()`, `arcopolis_export.cpp:327`)**. The melee HALF of doc 60 step 5 ONLY: a fight MECHANIC on one adjacency fixture — NOT a fight route (step 6 open), NOT the drone half (Spike 30 open), Q3 untouched, NO level-4 claim for the attack, NO kill (K=6 bumps vs the type-natural hp 80 is provably kill-safe: crit-inclusive worst case 6×12=72<80). **Zero `src/`**; tooling = an additive `--hp` flag on `make_monster_fixture.py` (defaults byte-identical, mechanically gated by the new G-ID check). RUNTIME-sandbox fixture `ArcopolisFightTest` (never committed) with **`SAFEMODE=false` pinned in the sandbox options** — safe mode is a RENDER-COUPLING artifact: INERT in headless tiles (`mon_info_update` early-returns on the CATA_SDL visibility-cache gate, `game.cpp:5310-5326`) but ACTIVE in a curses build, where it diverts every driven move at `avatar_action.cpp:363` with zero moves consumed; EVERY future hostile-in-view witness needs the same pin (doc 62 §5). Gated by `fight_mechanic_regression.ps1`; doc 62                                                                                          | ✅                                      |
...
[`docs/arcopolis/slice_regression.ps1`](slice_regression.ps1) gates the **folded Spike 28 vertical-slice
...
[`slice_live_driver.py`](slice_live_driver.py) (13/15 gates: possession-false-at-start, trajectory,
...
contact, proven off-contact displacement, hermeticity (G4 only — no z≤−2 state exists at 2 floors —
and count-pinned), and the literal `on_person_dialogue_predicate` scope guard, count-pinned). **Run with `pwsh`.** See
[61_SPIKE28_VERTICAL_SLICE_COMPOSITE.md](61_SPIKE28_VERTICAL_SLICE_COMPOSITE.md).
[`docs/arcopolis/fight_mechanic_regression.ps1`](fight_mechanic_regression.ps1) gates the **Spike 29A fight-MECHANIC
witness** (doc 62) on the RUNTIME-generated **`ArcopolisFightTest`** (a sandbox clone of `ArcopolisTest` with a
hostile `mon_zombie` adjacent one south at the type-natural hp 80 via the new `--hp` flag; nothing committed):
**G-ID** — a default-generation-flags invocation regenerates the reference `ArcopolisNearMonsterTest` `.sav`
byte-for-byte (SHA256, `.sav` scope — the generator's only content write; the fixture-isolation bound's
mechanical gate) → **G0** fixture pins
(`style_selected=="style_none"` — the knockback pin; `auto_travel_mode:false`; `manual_combat_mode` key absent
— a fixture-provenance pin: current saves DO write the key (`savegame.cpp:99`) and the loader resets it false
regardless (`game.cpp:3985`); avatar `"str_max":10` — the kill-safety premise; **`SAFEMODE=false` pinned in the
sandbox options.json** — the doc-62 render-coupling artifact, live via the `game.cpp:3984-3986` post-load
clobber) → **G1** across 3 seeds, non-live `[export, (move_s, export)×6]`: per-step
**turn-or-moves advance** (the safe-mode/auto-travel zero-cost-divert discriminator), avatar `pos_abs` held,
target holds its tile, monster count 1 in every export (join-by-elimination validity), hp non-increasing with
**≥1 drop** (RNG-dependent, the 27B pattern), zombie **alive at end** (kill-headroom 6×12=72<80), NPC Edwardo
PRESENT + stationary + held, transcript present + error-free (any prompt = attack-path failure). The zombie's counter-hits populate
`avatar.damage_taken[]` — reported, not gated. **Run with `pwsh`.** See
...
level 2/3; **the folded Spike 28 witnesses the matched-stair surface to a 6-floor round trip + the live
...
- **Richer commands:** **directed `examine` is implemented and runtime-proven (Spike 11A,
  ...
  **Vertical movement is no longer fully deferred — the `vertical_move` verb (`down`/`up` → the separate
  ...
  2/3); the folded Spike 28 (doc 61) WITNESSES a 6-floor round trip on `ArcopolisTowerTest` (per-leg
  snapshots — the witness doc 60's "5–6 floors" bar asked for; 6 is the witnessed count, no other N is
  claimed) plus the 2-floor and 6-floor slice composites, still at 2/3;**
