...
pain, thirst, fatigue, stored_kcal, kcal_percent, **`carried_items[]`** — Spike 25, **`damage_taken[]`** — Spike 27 Part 2) · `map_bounds` (origin_abs_sm[xyz], size_x, size_y,
...
Part 2) is the avatar's **attacker-attributed damage events since the prior snapshot** (drained per snapshot —
...
RNG-invariant like Part 1's position witness. See
[`57_SPIKE27_PART2_ATTACKER_DAMAGE.md`](57_SPIKE27_PART2_ATTACKER_DAMAGE.md).
...
exact tile/damage are RNG-dependent and NOT gated. The attacker-attributed damage fact is Part 2 (below). See
[56_SPIKE27_WORLD_TICK_LIVENESS.md](56_SPIKE27_WORLD_TICK_LIVENESS.md).
[`docs/arcopolis/attacker_damage_regression.ps1`](attacker_damage_regression.ps1) gates the **Spike 27 Part 2
...
SIBLING of the liveness script precisely so Part 1's RNG-invariant gates stay pristine). The funnel tap lives
...
- **Richer read-only export:** dynamic entities — **monsters done (Spike 6A, `entities.monsters[]`), NPCs
  ...
  **NOT** the `--arcopolis-new-world` generator the earlier note assumed. That generator + field state
  ...
  weather). The attacker-attributed damage half is **Spike 27 Part 2 — now built**
