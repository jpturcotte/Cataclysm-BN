...
'@ | Set-Content -Encoding ascii $scriptPath
    ...
    # a space (a spaced checkout/binary) would otherwise reach the exe's argument parser split into broken tokens.
...
if( $viewerExit -ne 0 ) {
    Write-Host "  FAIL: viewer exited $viewerExit (0=clean; 2=discrepancies incl. off-window items; 1=fatal). See $(Format-ArcoPath $verr) / $(Format-ArcoPath $report)." -ForegroundColor Red
    $fail++
}
if( -not $iw.Success ) {
