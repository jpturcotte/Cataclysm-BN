...
> audit's level-4 claims at the leaf — where we are *truly* with backend-input (sense-1) equivalence, per
> path — and triggered the three corrections folded in below: the `move` sense-2/3 caveat, the fail-loud
> "no silent *fake success*" vs. silent prompt-default distinction, and the vehicle-ground / WIELD
> witness-scope notes.
...
never a silent *fake success*. **One caveat (doc 38):** this guarantee covers fabricated success, NOT every
...
centralized **fail-loud** (exit 6) in both live and script paths, symmetric, with no silent *fake-success*
...
the audit establishes is only that the baseline is honest enough that *any* of these can be stated at the
...
The prerequisites for the *next* prompt-class exploration — **target undecided** — are in place and
honest:
...
- Every unsupported class is centralized **fail-loud**, so there is no silent *fake-success* path to
