...
| `string_input_popup`                     | **no gate**                                                                             | `src/string_input_popup.cpp`                      |
...
| furniture with actor   | `xfurn_t.examine` (`src/game.cpp:8692`), e.g. `iexamine::transform`/`workbench` — typically `uilist`-driven                                                                                             | menu (uilist)                      | possible                | possible                               | auto-cancel (gated) — **per-actor audit deferred**       |
...
> requested action is registered in the asking context; otherwise return `"QUIT"` if the context
> registered it — and if it did not, **hard-fail the session** rather than guess (fail-fast beats
> a silent hang). The hard-fail rule is a **future implementation policy proposal** for Spike 11A,
> not something this documentation PR implements.
...
3. **Direction vocabulary mapping.** The chooser consumes registered **input-context action ids**
   (`UP/DOWN/LEFT/RIGHT` + diagonals from `register_directions`, `src/input.cpp:994-1001`),
   not engine `action_id`s; `choose_direction` resolves them via `ctxt.get_direction()` **with
   `set_iso(true)`** (`src/action.cpp:1082, :1100`) — the protocol-cardinal → action-id mapping
   must be pinned against iso rotation at implementation time. Reserve a `here` token for the
   engine's `"pause"` self-tile path.
...
- **Exact failure mode while blocked:** `get_input_event` first calls
  `wrefresh( catacurses::stdscr )` when `needupdate` is set (`src/sdltiles.cpp:3962-3965`) and
  pumps `CheckMessages()` with curses uninitialized; whether that stays a pure block or can crash
  is not statically decided here. Either way it is fatal and unreachable-by-design under the
  guard.
- **Exact hook point inside `input_context::handle_input`** such that a served answer respects the
  context's registered-action set and filtering semantics (`src/input.cpp:925-992`) — an
  implementation-time decision.
- **Iso direction rotation:** `choose_direction` sets `set_iso(true)`; the cardinal → action-id
  mapping must be verified against `input_context::get_direction`'s iso handling.
- **Per-actor `iexamine` audit:** 40+ actors exist; only `none` and `locked_object` were read at
  the leaf. Each newly exposed target category needs its actor audited for raw reads before it is
  relied on. `vehicle::interact_with` (examine of a vehicle tile) is likewise unaudited.
- **Contexts without `"QUIT"`:** the hard-fail policy proposal covers them, but which contexts
  those are (and whether any sit on an examine path) is unenumerated.
...
the pickup tail), invisible to every existing backstop. What must be built first is small and
generic: a one-slot queued direction answer plus an auto-cancel guard at the
`input_context::handle_input` choke point, gated on the backend session — after which a one-shot
