...
6. **Hook point — concrete candidate, zero new engine accessors.** `input_context` already
   ...
   **[Spike 11A implementation correction:** those accessors sit inside the
   `#if defined(__ANDROID__)` block opening at `src/input.h:429` — they are NOT desktop-public,
   and MSVC rejects them. The shipped hook still adds zero engine API, by inversion: the hook call
   site is a _member_ of `input_context`, so it passes the private `category` and
   `registered_actions` into the backend directly. It also adds a `timeout >= 0` pass-through gate
   this record did not anticipate (the engine's activity-interrupt check polls a `DEFAULTMODE`
   context with `handle_input( 0 )` and registers no cancel action). See
...
| `is_action_registered`/`get_category` are existing public API — **corrected in Spike 11A: Android-only (`#if defined(__ANDROID__)` from `src/input.h:429`); see doc 26** | `src/input.h:469-478`                                                                                                                                   |
