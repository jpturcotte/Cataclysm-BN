...
// stateful file writer (begin/end_session_log) opens a real file and is exercised by the headless
// binary run against ArcopolisTest, not here, mirroring the backend-input test's split.

namespace {

/// True iff `s` is exactly one newline-terminated line (a single JSON Lines record): non-empty,
/// ends in
...
/// Parses one formatted record and runs `check` on the resulting object. JsonObject keeps a raw
/// JsonIn* (and reads members from the stream), so the istringstream + JsonIn must stay alive
/// across the reads -- hence the callback rather than returning the object. Folds in the shared
/// "one JSON Lines record with schema_version 1" assertions every event must satisfy.
template <typename Check> auto with_record(const std::string& line, Check check) -> void {
...
TEST_CASE("arcopolis session_start record is one valid JSON Lines object", "[arcopolis]") {
    ...
    arcopolis::write_session_start_line(
        out,
        {.world = "ArcopolisTest",
         .seed = std::nullopt,
         .export_dir = "out/arco",
...
TEST_CASE("arcopolis command record omits action_id when unresolved", "[arcopolis]") {
    ...
    arcopolis::write_command_line(
        out,
        {.step_index = 0, .command = "move", .direction = "move_e", .action_id = std::nullopt});
    with_record(out.str(), [](const auto& obj) { CHECK_FALSE(obj.has_member("action_id")); });
}

TEST_CASE(
    "arcopolis export record carries a pos_abs array and an integer step_index", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_export_line(
        out,
        {.step_index = std::optional<int>(2),
         .export_index = 1,
         .name = "after_move1",
...
TEST_CASE(
    "arcopolis export record writes a null step_index for the final snapshot", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_export_line(
        out,
...
TEST_CASE("arcopolis error record maps the kind to its name and exit code", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_error_line(
        out,
        {.step_index = std::optional<int>(4),
...
TEST_CASE("arcopolis session_end record carries counts and final state", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_session_end_line(
        out,
        {.status = "ok",
         .snapshots = 5,
         .commands = 3,
...
TEST_CASE(
    "arcopolis transcript is valid JSON Lines: every line parses as a JSON object", "[arcopolis]") {
    // Concatenate one of each event into a single buffer, exactly as the file accumulates them.
    std::ostringstream out;
    arcopolis::write_session_start_line(
        out, {.world = "W", .seed = std::nullopt, .export_dir = "d", .game_version = "v"});
    arcopolis::write_export_line(
        out,
        {.step_index = std::optional<int>(0),
         .export_index = 0,
         .name = "start",
         .path = "000_start.json",
         .final = false,
    ...
    // must parse as a JSON object carrying schema_version + event.
...
TEST_CASE("arcopolis session_start records the loaded autoselect option", "[arcopolis]") {
    // Spike 11A, doc 25 gate (h): the loaded AUTOSELECT_SINGLE_VALID_TARGET value is RECORDED
    // (never overridden) so examine witnesses are config-explicit.
...
TEST_CASE(
    "arcopolis nested_input_answer record carries context, direction and action", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_nested_input_answer_line(
        out,
        {.step_index = std::optional<int>(3),
         .context = "DEFAULTMODE",
         .direction = "move_n",
         .action = "UP"});
    with_record(out.str(), [](const auto& obj) {
...
TEST_CASE(
    "arcopolis nested_input_guard record carries the cancel, reason and fire count",
    "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_nested_input_guard_line(
        out,
        {.step_index = std::nullopt,
         .context = "PICKUP",
         .action = "QUIT",
...
TEST_CASE("arcopolis nested_input_unconsumed record explains the force-clear", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_nested_input_unconsumed_line(
        out,
        {.step_index = std::optional<int>(4),
         .direction = "move_w",
         .action = "LEFT",
...
TEST_CASE(
    "arcopolis prompt_opened record carries the kind and the engine's real menu choices",
    "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_prompt_opened_line(
        out,
        {.step_index = std::optional<int>(3),
         .kind = "menu",
         .choices = {
    ...
    with_record(out.str(), [](const auto& obj) {
        ...
        CHECK_FALSE(obj.has_member("witness")); // omitted when empty (menu/uilist records stay
                                                // byte-identical)
...
TEST_CASE(
    "arcopolis prompt_opened record carries the witness for a backend-driven prompt",
    "[arcopolis]") {
    // Spike 15: a witnessed backend-driven query_popup records WHICH audited call site armed it, so
    // the transcript proves the driven prompt was the witnessed one (the integration regression's
    // Gate Y2 then asserts this end to end). Emitted only when non-empty -- the menu/uilist case
    // above pins it absent.
    std::ostringstream out;
    arcopolis::write_prompt_opened_line(
        out,
        {.step_index = std::optional<int>(2),
...
TEST_CASE(
    "arcopolis prompt_answered record carries the multi-select choices and the served actions",
    "[arcopolis]") {
    // The level-4 artifact: a multi-select answer ([0,2]) becomes the exact registered-action
    // sequence the engine's own loop consumes -- one RIGHT mark per chosen entry, navigated forward
    // by DOWN, finalized by CONFIRM. Both arrays must round-trip in order.
    std::ostringstream out;
    arcopolis::write_prompt_answered_line(
        out,
        {.step_index = std::optional<int>(3),
    ...
    with_record(out.str(), [](const auto& obj) {
        ...
        // Spike 13B: kind unset (the old "PICKUP" menu) omits the field -- the existing wire is
        // byte-identical.
...
TEST_CASE(
    "arcopolis prompt_cancelled record carries the reason and omits an unknown step_index",
    "[arcopolis]") {
    std::ostringstream out;
    arcopolis::
        write_prompt_cancelled_line(out, {.step_index = std::nullopt, .reason = "no_channel"});
    with_record(out.str(), [](const auto& obj) {
        CHECK(obj.get_string("event") == "prompt_cancelled");
        CHECK_FALSE(obj.has_member("step_index"));
        CHECK(obj.get_string("reason") == "no_channel");
        CHECK_FALSE(obj.has_member("kind")); // Spike 13B: kind unset omits the field
                                             // (byte-identical)
    });
}

TEST_CASE(
    "arcopolis prompt_failed record carries reason and detail (prompt stays open)", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_prompt_failed_line(
        out,
        {.step_index = std::optional<int>(5),
         .reason = "bad_request",
...
TEST_CASE("arcopolis prompt_completed record carries the served-action count", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_prompt_completed_line(
        out, {.step_index = std::optional<int>(3), .actions_served = 5});
    with_record(out.str(), [](const auto& obj) {
...
TEST_CASE(
    "arcopolis prompt records carry the kind field for a backend-driven uilist", "[arcopolis]") {
    // Spike 13B: the vehicle-source uilist sets kind="uilist" on
    // prompt_answered/cancelled/completed so a reader can distinguish it from the old "PICKUP" item
    // menu (kind unset). Emitted only when non-empty.
    ...
    SECTION("prompt_cancelled") {
        std::ostringstream out;
        arcopolis::write_prompt_cancelled_line(
            out,
            {.step_index = std::optional<int>(2), .reason = "client_cancel", .kind = "uilist"});
        with_record(out.str(), [](const auto& obj) {
            CHECK(obj.get_string("event") == "prompt_cancelled");
    ...
    SECTION("prompt_completed") {
        std::ostringstream out;
        arcopolis::write_prompt_completed_line(
            out, {.step_index = std::optional<int>(2), .actions_served = 2, .kind = "uilist"});
        with_record(out.str(), [](const auto& obj) {
            CHECK(obj.get_string("event") == "prompt_completed");
...
TEST_CASE("arcopolis prompt_force_cancelled record names the kind and reason", "[arcopolis]") {
    // The vehicle pre-menu submenu (fail loud) and the in-activity secondary prompt (partial) are
    // each recorded so a force-cancel is never silent.
    std::ostringstream vehicle;
    arcopolis::write_prompt_force_cancelled_line(
        vehicle,
        {.step_index = std::optional<int>(4),
    ...
    arcopolis::write_prompt_force_cancelled_line(
        secondary,
        {.step_index = std::nullopt, .kind = "secondary_capacity", .reason = "partial pickup"});
