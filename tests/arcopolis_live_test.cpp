...
// drives a loaded game and stdin/stdout and is exercised by the headless binary against
// ArcopolisTest (docs/arcopolis/live_protocol_regression.ps1), mirroring the session-log test's
// split.
...
/// Parses one formatted protocol line and runs `check` on the resulting object. JsonObject keeps a
/// raw JsonIn* (and reads members from the stream), so the istringstream + JsonIn must stay alive
/// across the reads -- hence the callback. Protocol lines carry NO schema_version (the `ready`
/// event carries protocol_version instead), unlike the transcript's records.
template <typename Check> auto with_protocol_line(const std::string& line, Check check) -> void {
...
TEST_CASE("arcopolis live request parser accepts the four ops", "[arcopolis]") {
    // Spike 26A added "query" alongside the original export/command/quit; the structural-coverage
    // cases for the new op live in their own labeled TEST_CASE ("arcopolis op:query observes
    // on-person dialogue predicate; does NOT answer MGOAL_FIND_ITEM mission completion") so this
    // case stays focused on the three original command-shape ops.
    ...
    SECTION(
        "query (covered in depth by its own TEST_CASE; this case pins the four-op acceptance)") {
        const auto req = arcopolis::parse_live_request(
...
TEST_CASE(
    "arcopolis live request parser rejects structural problems as bad_request", "[arcopolis]") {
...
TEST_CASE("arcopolis live request parser whitelists the export name", "[arcopolis]") {
    ...
    const auto cases = std::array{
        ...
        std::string(R"({"id":1,"op":"export","name":")") + std::string(65, 'x') + R"("})", // too
                                                                                           // long
    };
...
TEST_CASE("arcopolis live success response carries the snapshot scalars", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_success_response_line(
        out,
        {.id = std::optional<int>(2),
         .op = "command",
...
TEST_CASE("arcopolis live success response omits the partial markers by default", "[arcopolis]") {
    // A clean pickup / any non-pickup command emits no forced_cancel/partial/unsupported_prompt
    // members, so existing wire output is byte-unchanged (the follow-up markers are strictly
    // additive).
    std::ostringstream out;
    arcopolis::write_success_response_line(
        out,
...
TEST_CASE(
    "arcopolis live success response marks a force-cancelled secondary prompt as partial",
    "[arcopolis]") {
    // ok stays true (a real partial pickup happened), but the explicit marker set makes the
    // partiality unmistakable so the result is never read as full success (docs/arcopolis/31).
    std::ostringstream out;
    arcopolis::write_success_response_line(
        out,
        {.id = std::optional<int>(5),
         .op = "command",
...
TEST_CASE("arcopolis live error response writes a null id and omits an unknown op", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_error_response_line(
        out,
        {.id = std::nullopt,
         .op = "",
...
TEST_CASE(
    "arcopolis parse_prompt_answer accepts a valid choice and an explicit cancel", "[arcopolis]") {
    ...
    // A `choices` array is multi-select, returned canonical: sorted + duplicate-free ([2, 0] -> [0,
    // 2]).
    ...
    const auto cancel =
        arcopolis::parse_prompt_answer(R"({ "op": "prompt_cancel", "id": 6, "prompt_id": 1 })", 3);
...
TEST_CASE(
    "arcopolis parse_prompt_answer rejects bad/missing prompt_id, out-of-range, duplicate, wrong-op and malformed",
    "[arcopolis]") {
    // Each is a RECOVERABLE bad_request (the caller rejects it, logs prompt_failed, keeps the
    // prompt OPEN).
    for (const std::string& line : {
             ...
             R"({ "op": "prompt_answer", "prompt_id": 1, "choices": [1, 2, 1] })", // duplicate
                                                                                   // (non-adjacent
                                                                                   // in input)
...
TEST_CASE(
    "arcopolis parse_prompt_answer rejects mid-prompt op:\"query\" as bad_request (Spike 26A parser-level mid-prompt witness)",
    "[arcopolis]") {
    ...
    const auto query_op_only =
        arcopolis::parse_prompt_answer(R"({ "op": "query", "prompt_id": 1 })", 3);
...
TEST_CASE("arcopolis live prompt event carries the real choices", "[arcopolis]") {
    std::ostringstream out;
    arcopolis::write_prompt_line(
        out,
        {.id = std::optional<int>(9),
         .prompt_id = 1,
         .kind = "menu",
...
TEST_CASE("arcopolis live prompt ack reports the chosen index/indices or a cancel", "[arcopolis]") {
    ...
    arcopolis::write_prompt_ack_line(
        chose, {.id = std::optional<int>(2), .prompt_id = 1, .choices = std::vector<int>{1}});
    ...
    arcopolis::write_prompt_ack_line(
        cancelled, {.id = std::optional<int>(3), .prompt_id = 1, .choices = std::nullopt});
...
// future doc/code drift would have to change all four coordinated sites to silently re-claim
// broader scope. The handler itself (run_live in arcopolis_live.cpp) needs a loaded world and is
// covered by the pwsh regression spike26a_dialogue_predicate_regression.ps1; the pure layer (parser
// + formatter + labeling-guard string) is covered here.

TEST_CASE(
    "arcopolis op:query observes on-person dialogue predicate; does NOT answer MGOAL_FIND_ITEM mission completion",
    "[arcopolis]") {
...
TEST_CASE(
    "arcopolis live query response carries the verbatim scope labeling guard", "[arcopolis]") {
    // The literal string "on_person_dialogue_predicate" is the labeling guard — it must appear in
    // the response payload BYTE-FOR-BYTE. Any future change to the scope label is the visible
    // signal of a scope-claim change (which would have to be reflected in the spike doc, the STATE
    // row, AND this assertion).
    std::ostringstream out;
    arcopolis::write_query_response_line(
        out,
        {.id = std::optional<int>(10),
...
TEST_CASE(
    "arcopolis live query response with has=false carries the same scope label", "[arcopolis]") {
    // The scope label is independent of the boolean — it is a property of the OP, not of the
    // answer.
    std::ostringstream out;
    arcopolis::write_query_response_line(
        out,
