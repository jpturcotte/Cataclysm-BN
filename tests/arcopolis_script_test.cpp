...
TEST_CASE(
    "arcopolis parse_script rejects op:\"query\" as bad_schema (Spike 26A: query is a live-only op)",
    "[arcopolis]") {
    // Spike 26A added op:"query" to the LIVE protocol parser (arcopolis_live.cpp
    // parse_live_request) but NOT to the script step parser (arcopolis_script.cpp ~:178-182, which
    // accepts op == "export" and op == "command" only). A scripted "query" step must therefore be
    // rejected at parse time as bad_schema -- there is no scalar-response channel in script mode,
    // so silently accepting it would be a hidden no-op. This case pins the symmetric rejection that
    // the doc 52 spike doc calls out.
    std::istringstream is(
...
TEST_CASE(
    "arcopolis parse_script accepts move steps with cardinal and diagonal directions",
    "[arcopolis]") {
    std::istringstream is(R"({ "schema_version": 1, "steps": [
...
TEST_CASE("arcopolis parse_script rejects a move step with a vertical direction", "[arcopolis]") {
    // Vertical (move_up/move_down) is the separate game::vertical_move primitive, not a planar
    // step.
    std::istringstream is(R"({ "schema_version": 1, "steps": [
...
TEST_CASE("arcopolis parse_script accepts vertical_move steps (down and up)", "[arcopolis]") {
    // Spike 24: the separate vertical verb is a normal command step (distinct down/up vocabulary),
    // so it parses in --arcopolis-run-script -- the matched-stair round-trip witness drives exactly
    // this.
    std::istringstream is(R"({ "schema_version": 1, "steps": [
...
TEST_CASE(
    "arcopolis parse_script rejects a vertical_move step without a direction", "[arcopolis]") {
    std::istringstream is(
...
TEST_CASE(
    "arcopolis parse_script rejects a vertical_move step with a non-vertical direction",
    "[arcopolis]") {
    // The vertical vocabulary is exactly down/up; planar-style tokens, the examine self-tile token,
    // and "" all reject at the script layer (its own coverage, though
    // is_supported_vertical_direction is shared).
    for (const std::string& dir : {"move_down", "move_up", "move_n", "here", ""}) {
        const auto json =
            R"({ "schema_version": 1, "steps": [
            { "op": "command", "command": "vertical_move", "direction": ")"
            + dir + R"(" }
        ] })";
...
// --- Spike 16: prompt_answers parsing (the smallest additive script format -- Option A).
// Structural validation only; semantic matching against the real opened prompt happens at runtime.
// ---

TEST_CASE(
    "arcopolis parse_script accepts pickup with a direction and a menu answer", "[arcopolis]") {
    std::istringstream is(R"({ "schema_version": 1, "steps": [
...
TEST_CASE(
    "arcopolis parse_script canonicalizes a menu choices array and a multi-prompt sequence",
    "[arcopolis]") {
    std::istringstream is(R"({ "schema_version": 1, "steps": [
        { "op": "command", "command": "pickup", "direction": "move_s",
          "prompt_answers": [
...
TEST_CASE(
    "arcopolis parse_script accepts a query_popup answer and a cancel answer", "[arcopolis]") {
    std::istringstream is(R"({ "schema_version": 1, "steps": [
...
TEST_CASE(
    "arcopolis parse_script rejects a prompt answer with both choice and cancel", "[arcopolis]") {
    std::istringstream is(R"({ "schema_version": 1, "steps": [
        { "op": "command", "command": "pickup", "direction": "move_s",
...
TEST_CASE(
    "arcopolis parse_script rejects a prompt answer with neither choice nor cancel",
    "[arcopolis]") {
    std::istringstream is(R"({ "schema_version": 1, "steps": [
        { "op": "command", "command": "pickup", "direction": "move_s",
...
TEST_CASE("arcopolis parse_script rejects a menu answer with duplicate choices", "[arcopolis]") {
    // codex PR#44 review: a duplicate index would ack as N picks but the resolver silently
    // sorts+uniques it (driving one selection), normalizing a malformed answer into a success. Live
    // mode rejects duplicates (src/arcopolis_live.cpp); the script parser must too, before the
...
TEST_CASE(
    "arcopolis parse_script rejects prompt_answers on a non-prompted command", "[arcopolis]") {
    std::istringstream is(R"({ "schema_version": 1, "steps": [
