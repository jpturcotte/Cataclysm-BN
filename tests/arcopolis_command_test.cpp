...
TEST_CASE("arcopolis parse_command accepts all eight planar move directions", "[arcopolis]") {
    ...
    for (const std::string& dir :
        ...
        const auto json =
            R"({ "schema_version": 1, "command": "move", "direction": ")" + dir + R"(" })";
...
TEST_CASE("arcopolis parse_command rejects vertical and garbage move directions", "[arcopolis]") {
    ...
    // `vertical_move` verb (Spike 24), NOT a planar `move` step; "east" is not an engine ident; ""
    // is empty. (Diagonals are now ACCEPTED -- see above.) This is the executable guard that the
    // planar/vertical split holds: `move` stays strictly planar, so the old planar-direction
    // negative probes (live/frontend recoverability) keep rejecting move_up/move_down.
...
TEST_CASE(
    "arcopolis is_supported_move_direction accepts the eight planar directions", "[arcopolis]") {
    for (const std::string& dir :
         {"move_n", "move_s", "move_e", "move_w", "move_ne", "move_nw", "move_se", "move_sw"}) {
...
TEST_CASE("arcopolis parse_command accepts examine with every supported direction", "[arcopolis]") {
    // All EIGHT planar directions the GUI examine chooser offers, plus "here" (self tile) -- the
    // backend must mirror the full chooser, not a cardinal-only subset.
    for (const std::string& dir :
         {"move_n", "move_s", "move_e", "move_w", "move_ne", "move_nw", "move_se", "move_sw",
          "here"}) {
        const auto json =
            R"({ "schema_version": 1, "command": "examine", "direction": ")" + dir + R"(" })";
        std::istringstream is(json);
        const auto result = arcopolis::parse_command(is);
        REQUIRE(result.has_value());
...
TEST_CASE("arcopolis parse_command rejects unsupported examine directions", "[arcopolis]") {
    ...
    // action id, not a protocol token: the protocol spells the self-tile "here". "move_n_e"/garbage
    // and empty are rejected. (Diagonals move_ne... are now ACCEPTED -- see the acceptance test
    // above.)
...
TEST_CASE(
    "arcopolis is_supported_target_direction accepts all 8 planar dirs plus here", "[arcopolis]") {
    for (const std::string& dir :
         {"move_n", "move_s", "move_e", "move_w", "move_ne", "move_nw", "move_se", "move_sw",
          "here"}) {
...
TEST_CASE(
    "arcopolis target_direction_nested_answer maps every direction to its chooser action id",
    "[arcopolis]") {
    ...
    // diagonal pairings are verified against get_direction (src/input.cpp): screen north=-y,
    // east=+x, so move_ne (north_east, +x,-y) -> "RIGHTUP", move_sw (south_west, -x,+y) ->
    // "LEFTDOWN", etc.
...
TEST_CASE("arcopolis parse_command rejects an unsupported pickup direction", "[arcopolis]") {
    ...
    std::istringstream is(
        R"({ "schema_version": 1, "command": "pickup", "direction": "move_up" })");
    const auto result = arcopolis::parse_command(is);
    REQUIRE_FALSE(result.has_value());
    CHECK(result.error().kind == arcopolis::command_error_kind::bad_schema);
}
...
TEST_CASE("arcopolis is_live_only_command flags only pickup", "[arcopolis]") {
    // pickup's core action needs the live prompt answer channel, so non-live modes FAIL LOUD for
    // it; every other verb completes in non-live mode and must NOT be rejected (examine still
    // examines; its tail auto-cancel is the engine's own "Never mind.").
    ...
    // vertical_move's matched-stair fast path is prompt-free, so it needs no answer channel: it
    // runs in
...
TEST_CASE("arcopolis parse_command accepts vertical_move down and up", "[arcopolis]") {
    ...
    for (const std::string& dir : {"down", "up"}) {
        const auto json =
            R"({ "schema_version": 1, "command": "vertical_move", "direction": ")" + dir + R"(" })";
        std::istringstream is(json);
        const auto result = arcopolis::parse_command(is);
        REQUIRE(result.has_value());
...
TEST_CASE("arcopolis parse_command rejects non-vertical vertical_move directions", "[arcopolis]") {
    // The vertical vocabulary is exactly "down"/"up". The planar-style "move_down"/"move_up"
    // tokens, a planar cardinal, the examine self-tile token "here", a bare compass word, and ""
    // are all rejected -- the parser half of the planar/vertical split.
