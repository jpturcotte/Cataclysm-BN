...
/// -> ACTION_EXAMINE, `pickup` -> ACTION_PICKUP), WITHOUT touching simulation state. This is the faithful
/// Spike 3.1 path: the engine's own switch( act ) in handle_action() computes the delta and calls
/// avatar_action::move / do_pause / examine() / pickup() at the input seam, so the backend never runs the
/// action itself. An examine/pickup direction is NOT encoded in the action_id -- it is the one-shot
/// nested-input answer served if the engine's own "Examine/Pickup where?" chooser asks (Spike 11A; see
/// arcopolis_backend_input.h). Returns the action_id, or a typed error for an unsupported command
/// (unsupported_command) or a bad direction (bad_schema). Pure: safe without a loaded world.
...
/// move_ne/move_nw/move_se/move_sw) plus "here" (the avatar's own tile -- the engine chooser's "pause"
/// path) -- exactly the planar target set a GUI player can pick at "Examine where?" / "Pickup where?".
/// Vertical (move_up/move_down) is rejected because game::examine and game::pickup pass
/// allow_vertical=false. Shared by the parsers and command_to_action. (Spike 11A; pickup reuses it in
/// Spike 12A -- both use the same choose_adjacent_highlight chooser.)
auto is_supported_target_direction( std::string_view ident ) -> bool;

/// Maps a supported target direction to the input-context ACTION ID the engine's direction chooser
...
/// This is the keystroke a GUI player would press at the "Examine where?" / "Pickup where?" prompt, NOT
/// an engine action_id and NOT a target tile. Returns nullopt for an unsupported ident. Pure. (Spike
